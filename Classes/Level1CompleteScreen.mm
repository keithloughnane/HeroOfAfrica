...
@implementation Level1CompleteScreen

+(id) scene
{
	CCScene *scene = [CCScene node];
	id child = [Level1CompleteScreen node];
...
-(id) init
{
	//NSString *str = [NSString stringWithFormat:@"%d",[GameNode getScore] ];
	...
	if( (self=[super init]) )
	{
		...
		CCSprite *background = [CCSprite spriteWithFile:@"level1complete.png"];
		...
		CCMenuItem * item0 = [CCMenuItemFont itemFromString:@"To Level 2" target:self selector:@selector(level1:)];
		...
		[temparr addObjectsFromArray:[[CCDirector sharedDirector] getScoreDetails:1]];
