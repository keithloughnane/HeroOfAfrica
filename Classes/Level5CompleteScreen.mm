...
@implementation Level5CompleteScreen

+(id) scene
{
	CCScene *scene = [CCScene node];
	id child = [Level5CompleteScreen node];
...
-(id) init
{
	if( (self=[super init]) )
	{
		...
		CCSprite *background = [CCSprite spriteWithFile:@"level5complete.png"];
...
-(void) buyScene:(Class)klass
{
	[self setLevelScene:[BuyScene class]];
}
/*
