...
@implementation Level3CompleteScreen

+(id) scene
{
	CCScene *scene = [CCScene node];
	id child = [Level3CompleteScreen node];
...
-(id) init
{
	if( (self=[super init]) )
	{
		...
		CCSprite *background = [CCSprite spriteWithFile:@"level3complete.png"];
		...
		CCMenuItem * item0 = [CCMenuItemFont itemFromString:@"To Level 4" target:self selector:@selector(level3:)];
		...
		[temparr addObjectsFromArray:[[CCDirector sharedDirector] getScoreDetails:3]];
