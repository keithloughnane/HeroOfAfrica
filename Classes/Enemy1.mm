...
-(void) update:(ccTime)dt
{
	if([self isOnScreen])
	{
		...
		//NSLog(@"speed %f",body_->GetLinearVelocity().x);
			[self updateFrames];
		CGPoint topLeft;
		CGPoint bottomRight;
		//
...
-(void) updateFrames
{
	...
	switch (behaveState) {
		case PATROLLEFT:
		{
			...
			//walk-left_02.png
			//running014.png
			NSString *str = [NSString stringWithFormat:@"walkingenemyfirst0%02d.png", x];
			CCSpriteFrame *frame = [[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:str];
			[self setDisplayFrame:frame];
		...
		case RUNLEFT:
		{
			...
			//walk-left_02.png
			//running014.png
			NSString *str = [NSString stringWithFormat:@"runenemyfirst0%02d.png", x];
			CCSpriteFrame *frame = [[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:str];
			[self setDisplayFrame:frame];
