...
{		
	// tiled
	CCTMXTiledMap *tiled = [CCTMXTiledMap tiledMapWithTMXFile:@"level1.tmx"];
	...
	//NSLog(@"Setting BG Vol to %f",vol);
	[[SimpleAudioEngine sharedEngine] setBackgroundMusicVolume:vol];
	//vol = [[CCDirector sharedDirector] getEffectsVolume];
		NSLog(@"Setting Sound Vol to %f",vol);
	...
	//playBackgroundMusic:(NSString*) filePath loop:(BOOL) loop
