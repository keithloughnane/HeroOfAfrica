...
@implementation Level2CompleteScreen

+(id) scene
{
	CCScene *scene = [CCScene node];
	id child = [Level2CompleteScreen node];
...
-(id) init
{
	if( (self=[super init]) )
	{
		...
		CCSprite *background = [CCSprite spriteWithFile:@"level2complete.png"];
		...
		CCMenuItem * item0 = [CCMenuItemFont itemFromString:@"To Level 3" target:self selector:@selector(level2:)];
		...
		[temparr addObjectsFromArray:[[CCDirector sharedDirector] getScoreDetails:2]];
